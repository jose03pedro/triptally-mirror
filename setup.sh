...
# Packing List Feature - Auto Setup Script
# Run this from the root of your project: /home/jose03pedro/MESW/LES/A

echo "🚀 Setting up Packing List feature..."

# Remove unnecessary folders
echo "🗑️  Removing triptally and triptally-1 folders..."
rm -rf triptally triptally-1

# Create directories
echo "📁 Creating directories..."
mkdir -p lib/packing
mkdir -p app/api/trips/\[id\]/packing/auto
mkdir -p types/packing

# 1. Create PackingItem Model
echo "📄 Creating PackingItem model..."
cat > app/models/PackingItem.ts << 'EOF'
...
echo "📄 Updating mongoose connection..."
cat > lib/mongoose.ts << 'EOF'
...
# 3. Create presets.ts
echo "📄 Creating packing presets..."
cat > lib/packing/presets.ts << 'EOF'
...
export function weatherBasedItems(
...
# 4. Create generatePacking.ts
echo "📄 Creating generation logic..."
cat > lib/packing/generatePacking.ts << 'EOF'
import {
  BASE_ITEMS,
  weatherBasedItems,
...
# 5. Create packing API route
echo "📄 Creating packing API route..."
cat > 'app/api/trips/[id]/packing/route.ts' << 'EOF'
...
export async function GET(
  ...
  try {
    ...
    // Check privacy - only owner can see packing list for private trips
...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await PackingItem.findOneAndDelete({
      _id: itemId,
      trip: id,
    });
...
# 6. Create auto-generate route
echo "📄 Creating auto-generate API route..."
cat > 'app/api/trips/[id]/packing/auto/route.ts' << 'EOF'
import { NextResponse } from "next/server";
import connectionToDB from "@/lib/mongoose";
import Trip from "@/app/models/Trip";
import PackingItem from "@/app/models/PackingItem";
...
import {
  ...
  WeatherDay,
} from "@/lib/packing/generatePacking";
...
export async function POST(
  ...
  try {
    ...
    // Try to fetch weather data if we have city and dates
    if (mainCity && startDate && endDate) {
      try {
        ...
        if (weatherRes.ok) {
          ...
          console.warn(
            "Weather API returned non-OK status, using base items only"
          );
        }
      } catch (weatherErr) {
        console.warn(
          "Failed to fetch weather, using base items only:",
          weatherErr
        );
      }
    }
    ...
    const existingNames = new Set(
      existingItems.map((item: any) => item.name.toLowerCase())
    );
    ...
    if (newItems.length === 0) {
      return NextResponse.json(
        {
          message: "Packing list is already up to date",
          itemsAdded: 0,
...
# 7. Create PackingList component
echo "📄 Creating PackingList component..."
cat > app/components/trip/packingList.tsx << 'EOF'
"use client";
  ...
  async function handleDeleteItem(itemId: string) {
    ...
    try {
      const res = await fetch(
        `/api/trips/${tripId}/packing?itemId=${itemId}`,
        { method: "DELETE" }
      );
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mb-0">
            Smart suggestions based on your trip destination and weather
            forecast.
          </p>
        </div>
      ...
      {totalCount > 0 && (
        <div className="mb-4">
          <div className="d-flex justify-content-between align-items-center mb-1">
            ...
            <span className="text-xs fw-medium text-slate-700">
              {progressPercent}%
            </span>
          </div>
        ...
        <div className="text-center py-5">
          ...
          {isOwner && (
            <p className="text-xs text-slate-400">
              Click &quot;Generate smart list&quot; to get personalized
...
cat > types/packing/types.ts << 'EOF'
export type PackingItemSource = "base" | "weather" | "profile";
...
echo ""
echo "✅ Packing List feature setup complete!"
echo ""
echo "📝 Next steps:"
echo "1. Update your trip page (app/trips/[tripId]/page.tsx) to import and use PackingListSection"
echo "2. Add this import: import { PackingListSection } from '@/app/components/trip/packingList';"
echo "3. Add this JSX where you want the packing list to appear:"
echo "   <PackingListSection tripId={tripId as string} isOwner={isOwner} />"
echo ""
echo "4. Run: npm run dev"
echo "5. Navigate to a trip page you own to test the packing list feature"
