...
export function TripSettingsSection({
  ...
  return (
    <div className="rounded-2xl bg-white shadow-sm border border-slate-100 p-4 md:p-5 space-y-4">
      {/* Trip Visibility */}
      <div>
          ...
          <div className="d-flex align-items-center gap-3 mt-3">
            <div className="form-check form-switch">
              <input
                ...
                disabled={toggling}
              />
              <label className="form-check-label text-sm" htmlFor="publicToggle">
        ...
        {isPublic && publicSlug && (
          <div className="mt-3 p-2 bg-light rounded">
            ...
            <div className="d-flex gap-2 align-items-center">
              <code className="text-xs flex-grow-1" style={{ wordBreak: "break-all" }}>
                {`${typeof window !== "undefined" ? window.location.origin : ""}/trips/public/${publicSlug}`}
              </code>
              <button onClick={copyPublicUrl} className="btn btn-sm btn-outline-primary">
                Copy
              </button>
            </div>
          </div>
      ...
      {isOwner && (
        <div className="pt-3 border-top">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h3 className="text-sm font-semibold text-slate-900 mb-0">Collaborators</h3>
            <button
          ...
          {/* Invite Form */}
          {showInviteForm && (
            <form onSubmit={handleInvite} className="card card-body bg-light mb-3 p-2">
            ...
            <>
              ...
              {invites.filter((i) => i.status === "pending").length > 0 && (
                <div className="mb-3">
                  ...
                  {invites
                    ...
                    .map((invite) => (
                      <div key={invite._id} className="d-flex justify-content-between align-items-center py-1">
                        <span className="text-xs text-slate-700">
                          {invite.invitedEmail || invite.invitedUserId?.email || "Unknown"}
                        </span>
              ...
              <div>
                ...
                {collaborators.length === 0 ? (
                  <p className="text-xs text-slate-400">Only you for now.</p>
                ) : (
                  collaborators.map((collab) => (
                    <div key={collab.user._id} className="d-flex justify-content-between align-items-center py-1">
                      <div className="d-flex align-items-center gap-2">
                        {collab.user.avatar ? (
                          ...
                          <div
                            className="rounded-circle bg-slate-200 d-flex align-items-center justify-content-center"
                      ...
                      <span className={`badge ${collab.role === "owner" ? "bg-primary" : "bg-secondary"}`}>
                        {collab.role}
                      </span>
                    </div>
                  ))
